...
class ForgotOtpForm extends Component{
	...
	submitOtp = (event) => {
		...
		if( x===1)
		{
			var mobile_number = localStorage.getItem('m');
			var user_type = localStorage.getItem('u');
			var data = {from:'web',otp:parseInt(this.state.otp,10),mobile_number: parseInt(mobile_number,10),user_type: parseInt(user_type,10)};
			callApi('forgot_verify_otp','post',data).then(response => {
		        if(response.status === 200)
		        {
		         	localStorage.setItem('m',response.data.mobile_number);
		         	localStorage.setItem('u',response.data.user_type);
		         	localStorage.setItem('i',response.data._id);
		         	if(response.page === 'reset_password')
		         	{
...
export default connect(mapStateToProps)(ForgotOtpForm);